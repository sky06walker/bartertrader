  ...
  async function handleSocialLogin(provider) {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setToast('An account already exists with the same email. Try a different login method.');
    ...
    <div className="register-page-wrapper">
      <div className="register-page">
        <div className="register-container">
          ...
          <img src={logoImg} alt="BarterTrader Logo" className="register-logo-img" />
          <h1 className="register-brand" style={{ display: 'none' }}>BarterTrader</h1>
          ...
          <form className="register-form glass-card" onSubmit={handleSubmit}>
            <h2 className="register-form-title">
              {mode === 'signup' ? 'Create Account' : mode === 'signin' ? 'Sign In' : '🔐 Forgot Password'}
            </h2>
            ...
            {mode === 'forgot' && resetSent ? (
              <div style={{
                ...
                <p style={{ fontSize: 'var(--font-size-3xl)', marginBottom: 'var(--space-sm)' }}>📧</p>
                <p style={{ color: 'var(--color-success)', fontWeight: 600 }}>Reset email sent!</p>
                <p style={{ color: 'var(--color-text-muted)', fontSize: 'var(--font-size-sm)', marginTop: 'var(--space-xs)' }}>
                  Check your inbox for <strong>{form.email}</strong> and follow the link to reset your password.
                </p>
                <button
                  type="button"
              ...
              <>
                {mode === 'signup' && (
                  <>
                    <div className="form-group">
                      ...
                      <input className="form-input" type="text" name="name" placeholder="Enter your name" value={form.name} onChange={handleChange} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Phone Number <span className="required">*</span></label>
                      <input className="form-input" type="tel" name="phone" placeholder="012-3456789" value={form.phone} onChange={handleChange} />
                    </div>
                  </>
                )}
                ...
                {mode === 'signin' && (
                  <div style={{ textAlign: 'right', marginTop: '-8px', marginBottom: 'var(--space-sm)' }}>
                    <button
                      type="button"
                ...
                <button className="btn btn-primary register-btn" type="submit" disabled={submitting}>
                  {submitting
                    ? '⏳ Please wait...'
                    : mode === 'signup'
                      ? 'Get Started 🚀'
          ...
          <div className="auth-footer" style={{ textAlign: 'center', marginTop: 'var(--space-xl)', fontSize: 'var(--font-size-sm)', color: 'var(--color-text-muted)' }}>
            <p>
              By continuing, you agree to our <br/>
              <Link to="/privacy-policy" style={{ color: 'var(--color-accent)', textDecoration: 'none' }}>Privacy Policy</Link> &bull; <Link to="/data-deletion" style={{ color: 'var(--color-accent)', textDecoration: 'none' }}>Data Deletion</Link>
            </p>
          </div>
        </div>
